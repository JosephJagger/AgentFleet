...
| Credit balance | The current account credit/point balance returned by native Codex. It is an account-level balance, not a token estimate and not a project allocation. |
...
The authenticated read-only endpoints are `/api/machines/:id/usage`, `/api/projects/:id/usage`, and `/api/sessions/:id/usage`. Each enforces workspace ownership. Credit snapshots require control-plane schema 34; take a consistent backup before upgrading. Rolling back to an older binary requires restoring its matching backup.

## 中文说明

顶部“设置”前有独立的“消耗”页面。选择主机后，可以按项目和会话查看总消耗、本周消耗和周缓存命中率，并默认按本周消耗排序；点击会话名称可直接进入工作台。项目与会话原有的用量入口也显示相同口径。账号展示原生 Codex 上报的已用比例、剩余比例、重置时间、当前点数余额和更新时间；点数是账号级余额，不按 token 折算，也不分摊到项目。缓存和推理明细已经包含在对应总量里，不能重复相加。
...
同账号额度和点数不能跨主机相加，也不能按 token 比例分摊到项目。本周数据严格使用 Codex 上报的 10,080 分钟周额度周期；周命中率为该周期内“缓存输入 token ÷ 输入 token”。总消耗不展示累计命中率。数据库 schema 28 开始按观测区间保存输入和缓存输入明细，旧区间仍计入 token 总数，但周命中率显示“未记录”，不会估算；点数快照需要 schema 34。
