...
export function SessionInspector({ detail, loading, draftOwner, onLoadHistory, historyLoading, onRefresh, onClaim, onReleaseManagement,  onSend, onQueue, onSteer, onCancelQueued, onCancel, onApproval, onClose, onNewSession }: {
  ...
  const closeAddMenuAfterTouchChoice = () => {
    if (window.matchMedia?.("(max-width: 900px), (pointer: coarse)").matches) closeAddMenu();
  };
  ...
  const aiOptimizeButton = completionPreferences.suggestions && <button type="button" className="button button--secondary composer-ai-button" aria-label={aiBusy ? t("正在优化…") : t("优化表达")} title={t("保留原意，让描述更清晰、专业")} disabled={aiBusy || busy || !prompt.trim() || Boolean(slashCommand)} onClick={async()=>{
          ...
          }}>{aiBusy ? <LoaderCircle className="spin" size={16} /> : <Sparkles size={16} />}<span className="composer-action-label composer-action-label--full" aria-hidden="true">{aiBusy ? t("正在优化…") : t("优化表达")}</span><span className="composer-action-label composer-action-label--compact" aria-hidden="true">{aiBusy ? t("优化中") : t("优化")}</span></button>;
