...
it("跨页全选有图会话，空选不派发；逐会话失败跳过，清理需确认",async()=>{
  ...
  expect(mocks.imageOperation).toHaveBeenCalledTimes(12);
  fireEvent.click(screen.getByRole("button",{name:"确认清理附件"}));
  await screen.findAllByText(/附件已清理/);
...
it("失败不会显示已清理，新主机重新加载独立范围",async()=>{
  ...
  await screen.findByText("清理结果待核验，不会自动重试");expect(screen.queryByText(/附件已清理/)).toBeNull();
