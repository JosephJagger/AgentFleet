...
export function PermissionPanel({ machineId, sessionId, observed }: { machineId?: string; sessionId?: string; observed?: RuntimeSettings | null }) {
  ...
  const machineSummary = data ? `${permissionNames[data.profile]} · ${permissionSources[data.source]}` : t("读取配置中");
  return machineId ? <section className="permission-panel permission-panel--host" aria-label={t("主机默认权限")}><details className="host-config-disclosure"><summary><span><strong>{t("主机默认权限")}</strong><small>{machineSummary}</small></span></summary><div className="host-config-disclosure__body">{content}</div></details></section> : <details className="permission-panel session-config-section"><summary><span>{t("执行权限")}<small>{data ? `${permissionNames[data.profile]} · ${effective}` : t("读取配置中")}</small></span></summary>{content}</details>;
}
