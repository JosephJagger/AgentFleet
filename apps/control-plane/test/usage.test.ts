...
test("usage routes require authentication and migration creates version 29",async t=>{
 const {app,db}=await buildControlPlane(config());t.after(()=>app.close());
 assert.equal(db.get<{user_version:number}>("PRAGMA user_version")?.user_version,29);
