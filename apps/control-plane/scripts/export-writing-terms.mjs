...
const rewrites = corpus.intents.filter(intent => ['philosophy','cognition','ai','agent','ecommerce','analytics','operations','marketing','finance','product','design','research'].includes(intent.domain) || ['programming.testing','programming.devops','programming.integration','programming.mobile','programming.security','office.documents'].includes(intent.category) || ['video-j-cut','video-l-cut','video-ducking','video-speed-ramp','video-subtitle-timing'].includes(intent.id)).map(intent => ({id:intent.id,domain:intent.domain,category:intent.category,examples:intent.examples,goals:intent.goals}));
