...
Codex CLI 本身已经支持通过 `--image` 和交互式粘贴输入图片，AgentFleets 将这一使用流程带到浏览器，并连接到已配对的远端主机。参见 [Codex CLI 官方文档](https://learn.chatgpt.com/docs/codex/cli)。这里的多模态提交指**文字与图片**；输入框目前尚未提供通用文件选择器、拖拽上传和音视频提交。Codex 在回复中链接的项目文件，可以按需从执行主机预览或下载。
